function results = test_sctrca(eeg, model, is_ensemble, fs, list_freqs, pha_val, alpha, beta)
    ...
    for fb_i = 1:1:model.num_fbs
        ...
        for class_i = 1:1:model.num_targs
            ...
            if ~is_ensemble
                ...
                w1 = w(1:num_chans);
                w2 = w(num_chans+1:end);
            else
                w = squeeze(model.W(fb_i, :, :))';
                w1 = w(1:num_chans,:);
                w2 = w(num_chans+1:end,:);
            end
            Ref=ref_signal_nh(list_freqs(targ_i),fs,pha_val(targ_i),num_smpls,5);
            %% standard scTRCA
            r_tmp1 = corrcoef(testdata'*w1, traindata'*w1);
            r_tmp2 = corrcoef(testdata'*w1, Ref'*w2);
            r_tmp = sign(r_tmp1(1,2))*r_tmp1(1,2).^2+sign(r_tmp2(1,2))*r_tmp2(1,2).^2;
            r(fb_i,class_i) = r_tmp;
           % weighted scTRCA
           for w_i = 1:size(w, 2)
                r_tmp1 = corrcoef(testdata'*w1(:,w_i), traindata'*w1(:,w_i));
                r_tmp2 = corrcoef(testdata'*w1(:,w_i), Ref'*w2(:,w_i));
                tmp = sign(r_tmp1(1,2))*r_tmp1(1,2).^2+sign(r_tmp2(1,2))*r_tmp2(1,2).^2;
                r_tmp = [r_tmp, tmp];
            end
            w_tmp = 1:size(w, 2);
            ...
            r_tmp1 = corrcoef(testdata'*w1, traindata'*w1);
            r_tmp2 = corrcoef(testdata'*w1, Ref'*w2);
            r_tmp11 = sign(r_tmp1(1,2))*r_tmp1(1,2).^2+sign(r_tmp2(1,2))*r_tmp2(1,2).^2;
            for w_i = 1:size(w, 2)
                r_tmp1 = corrcoef(testdata'*w1(:,w_i), traindata'*w1(:,w_i));
                r_tmp2 = corrcoef(testdata'*w1(:,w_i), Ref'*w2(:,w_i));
                tmp = sign(r_tmp1(1,2))*r_tmp1(1,2).^2+sign(r_tmp2(1,2))*r_tmp2(1,2).^2;
            ...
            r2(fb_i,class_i) = max(r_tmp11(1,1));
